...
# ╔═╡ 09906f72-f0c0-11ef-2e19-a72a6fc6f270
using Distributions

# ╔═╡ d1ac3b19-9466-4179-a7b3-1f51e0bf0f4e
using Turing

# ╔═╡ e1844fde-5454-465d-a318-61fd5e5605f7
using Plots, StatsPlots

# ╔═╡ a9e16207-8268-489f-a6ff-122af99fe1cc
using PlutoUI

# ╔═╡ 02aa4b5d-816a-4b18-aad7-7f51323098fb
function growth_curve(t, t₀, C₁, C₂, D₁, D₂)
	if t < t₀
		return 0
	end
	1 / (C₁*exp(t/D₁) + C₂*exp(-t/D₂))
end

# ╔═╡ 02d7d326-5ffa-49f4-94fb-bf73bfea84e9
@model function growth_model(ts, y, σ)
	t₀ ~ Cauchy(0, 2.5)
	C₁ ~ Uniform(0, Inf)
	C₂ ~ Uniform(0, Inf)
	D₁ ~ Uniform(0, Inf)
	...
	g = growth_curve.(ts, t₀, C₁, C₂, D₁, D₂)
	for i in eachindex(y)
		y[i] ~ Truncated(Normal(g[i], σ), 0, Inf)
	end
	# y ~ product_distribution(Truncated.(Normal.(g, σ), 0, Inf))
	return y
end

# ╔═╡ e7181328-4dcb-4d7a-91d0-6108209a2ffe
md"""
From a BED perspective, it may not make much sense to design around the full version of the model. Because it has so much freedom, there's basically a lot of possibilities you would have to design around, when often you are more interested in designing for a particular sub-model.

For example, a logistic model allowing both growth and decay can be recovered from the model by allowing either $D_1 \rightarrow \infty$ or $D_2 \rightarrow \infty$. This should be reparameterized into the more familiar form with parameters $(L, q_0, r)$, where $r$ can be positive or negative, through a simple mapping:
"""

# ╔═╡ 1fab27de-21e2-43ce-a6ad-0ccecb62e7e4
function logistic_growth_curve(t, t₀, r, P₀, L)
	if t < t₀
		return 0
	end
	a = L/P₀ - 1
...
# ╔═╡ 2f8b5ed4-2bac-45a1-9b8b-48395241384d
@bind r_slide Slider(-1:0.1:1)

# ╔═╡ b88779c2-2628-4edc-9ba3-643bcefd0c0d
r_slide

# ╔═╡ 329fa980-1913-451a-9071-88b64fac9c5a
md"""
Let's make sure the two parameterizations match

TODO ...work out what the problem is
"""

# ╔═╡ 1bd4acf8-d330-4c21-a1bc-434ffa4b4426
let P₀=1e-3, L=2
	g1 = logistic_growth_curve.(0:50:300, 0, r_slide, P₀, L)
	g2 = growth_curve.(0:50:300, 0, 1/L, (L/P₀ - 1)*L, Inf, 1/r_slide)

	all(g1 .== g2)
end

# ╔═╡ ac56d945-0573-4151-bc3f-5f044471e404
plot(0:200, growth_curve.(0:200, 0, 1/2, (2/1.5 - 1)/2, Inf, 1/r_slide))

# ╔═╡ c46beb64-c850-4dbe-8671-1a9daae38f9c
1/1.5-1

# ╔═╡ f98fe887-5293-4ab2-8a30-7b2bf4480a52
@model function logistic_growth_model(ts, y, σ)
	t₀ ~ Cauchy(0, 2.5)
	L ~ Uniform(0, Inf)
	C₂ ~ Uniform(0, Inf)
	D₂ ~ Uniform(0, Inf)

	g = growth_curve.(ts, t₀, L, C₂, Inf, D₂)
	for i in eachindex(y)
		y[i] ~ Truncated(Normal(g[i], σ), 0, Inf)
	end
	# y ~ product_distribution(Truncated.(Normal.(g, σ), 0, Inf))
...
# ╔═╡ 01747bef-654d-4411-af25-d76157a83cbd
begin
	ts = 0:240
	gtrue = growth_curve.(ts, 5, 1, 2, Inf, 1/0.07)
	σ = 0.05
	y = rand.(truncated.(Normal.(gtrue, σ), 0, Inf))
	plot(ts, gtrue)
	scatter!(ts, y)
end

# ╔═╡ 3b78835b-69cb-4847-b35c-bc713e30274d
begin
	gm_pri_pred = growth_model(ts, fill(missing, length(ts)), σ)

	p = plot(;title="posterior prediction", legend=false)
	for i in 1:10
		plot!(p, ts, gm_pri_pred())
	end
	p
end

# ╔═╡ 4c3f7d78-161b-42ad-8b37-d23c71a482e5
begin
	chn_pri = sample(gm_pri_pred, Prior(), 1000)
	
end

# ╔═╡ d25ea939-4528-4e86-8a6d-a1b26731cf0a
fill(nothing, 10)

# ╔═╡ 64aee6d2-7e64-4346-957a-a50e58cdd68f
Truncated
...
manifest_format = "2.0"
project_hash = "711103e681e6593fe687137e1891730b37a211dc"
...
git-tree-sha1 = "2e4b08a9adf6aba7c3239f56c7443096eff8f361"
