...
find_tmax <- function(qmax, t0, q0, r) {
    a <- 1/q0 - 1
    ceiling(t0 - log((1-qmax)/(a*qmax)) / r)
}
...
obs_info_mat <- function(a, eterm) {
    q <- 1 / (1 + a*eterm)
    diag(1 / (q*(1-q)))
}

info_mat <- function(tmax, t0, q0, r) {
    a <- 1/q0 - 1
    if (t0 < 0) { # TODO this case relatively untested. 6/4 Confirm e.g. there's no discont. when t0->0
        warn("t0 < 0 is untested")
        ts <- seq(-t0, -t0+tmax, 1)
    } else {
    ...
    O <- obs_info_mat(a, eterm)
    return(t(J) %*% O %*% J)
}

# Maximum likelihood estimation and Monte Carlo variance--------------------------
